...
`--install` 사용 시 연결된 기기의 ABI를 자동 감지하여 해당 아키텍처만 빌드합니다 (예: `arm64-v8a`만 빌드 → 빌드 시간 약 50% 단축). `--install` 없이 빌드하면 전체 ABI(arm64-v8a, armeabi-v7a)로 빌드됩니다.
...
블루투스 오디오 장치가 연결된 경우 자동으로 BT 마이크로 입력을 받습니다.

| 상태 | 입력 소스 |
|------|-----------|
| BT 미연결 | 기본 폰 마이크 |
| BT 연결 | 블루투스 마이크 (SCO) |
| BT 연결 해제 시 | 자동으로 폰 마이크 복귀 |

녹음 시작 시 `startBluetoothSco()`로 SCO 연결을 시도하고, 녹음 종료 시 자동 해제됩니다. BT 장치가 없으면 무시되고 기본 마이크가 사용됩니다.
...
3. 녹음 → Stop → Slack 채널에 타임스탬프 포맷으로 메시지 수신
...
[0:04] 왜 그러세요?
```
문장 단위(`.` `?` `!`)로 분리되며, 각 문장의 시작 시각이 `[분:초]` 형태로 표시됩니다.
